...
// Here, after form submission, handle the email and password validation
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($customer) {
        // 이미 존재하는 이메일입니다.
        echo "<script>alert('이미 존재하는 이메일입니다.'); return false;</script>";
    } else {
        // Compare password
        ...
        if($customer['password'] != $password) {
            // 비밀번호가 일치하지 않습니다.
            echo "<script>alert('비밀번호가 일치하지 않습니다.'); return false;</script>";
        } else {
            // Your code to update customer information
        }
    }
}
?>
    <div class="container">
        <form name="customer_form" action="<?=$action?>" method="post" class="fullwidth">
            ...
            <p>
                ...
                <input type="text" placeholder="고객 이름 입력" id="name" name="name" value="<?=$name ?>"/>
            </p>
            <p>
                <label for="contact">연락처</label>
                <input type="number" placeholder="정수로 입력 (-은 제외)" id="contact" name="contact" value="<?=$contact?>" />
            </p>
            <p>
                <label for="email">Email*</label>
                <input type="text" placeholder="이메일은 필수 입력" id="email" name="email" value="<?=$email?>" />
            </p>
            <p>
                <label for="address">주소</label>
                <input type="text" placeholder="주소 입력" id="address" name="address" value="<?=$address?>" />
            </p>
            <p>
